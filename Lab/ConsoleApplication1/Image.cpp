...
private:
    ...
    void generateStride() {
        /* Довольно стремная логика вычисления через последовательное
         * деление и умножение на одно и то же. Следовало воспользоваться
         * оператором взятия остатка. А то шаманство какое-то */
...
public:
    ...
    void writeBMP(string filename) {
        ...
        /* А разве важно, какого она размера? Просто запиши её. Мы же уверены теперь, что 
         * она к правильной картинке относится  */
        if (myInfo.biSize == 108 || myInfo.biSize == 124) {
        ...
        /* А почему j за пределом цикла определила? */
        int k = 0; int j = 0;
        for (int i = 0; i < myInfo.biHeight; i++) {
            for (j; j < 3 * myInfo.biWidth * (i + 1); j++) {
                buffer2[k] = buffer[j];
    ...
    /* Мне кажется, можно было бы картинку и на стеке хранить, 
     * А то кто потом будет не забывать их удалять? */
    Image* rotate(int angle) {
        ...
        switch (angle) {
        /* Эта идея с градусами всё еще мне не нравится. Тут нужен enum class тогда,
         * либо делить на две функции */
        case(90):
    ...
    Image* blur_collapsed(int radius, double sigma) {
        ...
        for (i = 0; i < bmp->myInfo.biHeight; i++) {
            for (j = 0; j < bmp->myInfo.biWidth; j++) {
                ...
                a2 = 0;
                a3 = 0;
                /* Больно крупная вложенность, можно было бы разделить с помощью 
                 * вспомогательной функции или как-то ещё */
